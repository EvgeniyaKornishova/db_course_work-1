<template>
  <div class="fill-height">
    <v-app-bar dark prominent src="@/assets/beans.jpg">
      ...
      <v-app-bar-title class="ml-6 display-2" width="100%">
        <div>Cup of coffee</div>
      </v-app-bar-title>

      <v-row v-if="!user" class="mt-5 ma-0" justify="end">
        <v-btn class="mr-4" to="/login">
          войти
        </v-btn>
        <v-btn class="mr-2" to="/registration">
          регистрация
        </v-btn>
      </v-row>
      ...
      <v-row v-else justify="end" class="ma-4">
        <v-menu bottom min-width="150px" rounded offset-y>
          <template v-slot:activator="{ on }">
            <v-btn icon v-on="on">
              <v-avatar class="mt-4" color="brown lighten-5" rounded size="77">
                <v-img src="@/assets/cup.png"></v-img>
              </v-avatar>
            </v-btn>
          </template>
          <v-card class="pa-2">
            ...
            <div v-if="isBarista">
              <v-btn depressed text to="/barista">
                Бариста
              </v-btn>
              <v-divider class="my-2"></v-divider>
            </div>
            <div v-if="isBarista">
              <v-btn depressed text to="/admin">
                Админ
              </v-btn>
              <v-divider class="my-2"></v-divider>
            </div>
      ...
      <template v-slot:extension>
        <v-tabs v-model="tab" align-with-title background-color="#00000070">
