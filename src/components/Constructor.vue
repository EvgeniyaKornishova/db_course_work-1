      ...
      <v-col cols="7">
        ...
        >
          <template v-slot:item.delete="{ item }">
            <v-btn @click="deleteIngredient(item)" small outlined>
        ...
        <v-btn dark color="brown lighten-1" @click="createCoffee()">
          Создать
        </v-btn>
      </v-col>
  ...
  methods: {
    getingredients() {
      axios({
        ...
        this.ingredients = data.map((elem) => {
    ...
    createCoffee() {
      axios({
        ...
        data: {
          components: this.coffee.map((item, i) => ({
            addingOrder: i,
    ...
    addProduct(item) {
      ...
      if (!found) this.coffee.push(item);
