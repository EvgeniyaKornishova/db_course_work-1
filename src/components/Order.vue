...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      finalCost (){
        return this.order.map(item => +item.costNumber * +item.amount).reduce((acc, cur) => acc+cur,0);
      },
      panels(){
        return {
          coffee: {
    ...
    methods: {
      getCoffee(){
        ...
        .then(response => {
          ...
          this.panels.coffee.items = data.map(
            elem => {
      ...
      getDesserts(){
        ...
        .then(response => {
          const { data } = response;
           ...
           this.panels.desserts.items = data.map(
            elem => {
              let { id, name, cost } = elem;
              let costNumber = cost;
              cost = `${cost} р`;
      ...
      getFevoriteCoffees(){
        ...
        .then(response => {
          ...
          this.panels.favoriteCoffees.items = data.map(
            elem => {
              let { id, name, cost } = elem;
              let costNumber = cost;
              cost = `${cost} р`;
